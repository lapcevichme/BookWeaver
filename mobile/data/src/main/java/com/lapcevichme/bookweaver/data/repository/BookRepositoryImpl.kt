...
import com.lapcevichme.bookweaver.data.WsBookList
import com.lapcevichme.bookweaver.data.WsMessage
import com.lapcevichme.bookweaver.data.WsRequestBookList
import com.lapcevichme.bookweaver.data.network.dto.toDomain
    ...
    private fun handleIncomingMessage(messageString: String) {
        try {
            when (val message = json.decodeFromString(WsMessage.serializer(), messageString)) {
                is WsBookList -> {
                    _books.value = message.books.map { it.toDomain() }
                }
